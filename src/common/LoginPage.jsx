...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (user) {
      // Assuming the user object contains an 'isAdmin' property
      if (user.isAdmin) {
        ...
        <div className="max-w-[480px] w-full">
          <div className="p-6 sm:p-8 rounded-2xl bg-white border border-gray-200 shadow-sm">
            ...
            <form onSubmit={handleSubmit} className="mt-12 space-y-6">
              ...
              {status === 'failed' && (
                  <p className="text-sm text-red-600 text-center">{error?.message || 'Login failed. Please check your credentials.'}</p>
              )}
